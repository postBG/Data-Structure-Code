		...
		while (true)
		{
			try
			{
				...
				if (input.compareTo("QUIT") == 0)
					...
					Table.retrievalPattern(pattern);*/
					...
					while((tmpString=BrForFile.readLine())!=null){
						...
						//패턴의 길이씩 읽은 문장을 잘라서 그것이 만일 pattern을 포함하고 있으면 그 좌표를 저장하고 아니면 저장하지 않고 넘어감
						for(int i=0; i<tmpString.length()-PatternLength+1; i++){
							Tmp=String.copyValueOf(TmpArray, i, PatternLength);
							col=i+1;
					...
					if(LineList.size()!=0){
						int i=0;
						Iterator<Integer> LineIter=LineList.iterator();
						Iterator<Integer> ColIter=ColList.iterator();
						
						while(LineIter.hasNext()){
							if(i!=LineList.size()-1){
								System.out.print("("+LineIter.next()+", "+ColIter.next()+") ");
								i++;
							}
							else{
								System.out.println("("+LineIter.next()+", "+ColIter.next()+")");
		...
		for(int i=0; i<NumberOfHashValue; i++){
			...
			if(i==0){//맨 처음의 경우
				...
				for(int j=list.size()-1; j>=0; j--){
					ListNode TmpListNode=list.peekLast();
					int flag=0;
