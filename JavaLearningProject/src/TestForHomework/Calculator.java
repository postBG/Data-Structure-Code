	...
	public static void main(String[] args) {
		...
		while (true)
		{
			...
			{	
				...
				if(checkSpaceBetweenNumber(infix))//숫자 사이에 공백이 들어가는 경우
					flag=1;
	...
	private static double calculateWithPostfix(StringBuffer postfix, StackForCalculate stacknum){
		...
		for(int i=0; i<LenthOfPostfix; i++){
			...
			//단항연산자와 숫자문자의 경우 number에 넣는다.
				...
				double x=Double.parseDouble(NumberString);//number를 실수형 숫자로 바꾸어준다.
				i=i+cnt;
		...
		double answer=stacknum.pop();//마지막에 stack에 남아있던 것이 답 
		return answer;
